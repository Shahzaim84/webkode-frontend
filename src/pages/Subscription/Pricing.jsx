...
const Pricing = () => {
  ...
  const PricingCard = ({ plan }) => {
    ...
    return (
      <div
        className={`relative flex flex-col p-8 bg-gradient-to-br ${plan.color} rounded-2xl border-2 cursor-pointer w-80 ${
          plan.popular 
            ? 'border-red-300/80 scale-[1.02]' 
            : 'border-gray-200/80'
      ...
      >
        ...
        <div className="mb-6">
          <h3 className="text-3xl font-bold text-gray-900 mb-2">{plan.name} {plan.name === 'Premium' && <FaCrown className="w-6 h-6 text-amber-500" />}</h3>
        ...
        <button
          className={`w-full flex justify-center items-center gap-2 py-4 rounded-xl font-semibold transition-all duration-300 cursor-pointer ${
            plan.popular
        ...
        >
          Subscribe {plan.popular && <FaFire className="w-4 h-4 text-amber-200 animate-pulse" />}
        </button>
      </div>
    );
  };
  ...
  return (
    <div ref={container} className="min-h-screen bg-gradient-to-br from-gray-50/50 to-white">
      <div className="max-w-7xl mx-auto px-4 py-20">
        <div className="hero-heading text-center mb-20">
          <h1 className="text-5xl font-bold text-gray-900 mb-6 leading-tight">
            Simple pricing for <span className="bg-gradient-to-r from-red-500 to-red-600 bg-clip-text text-transparent">powerful tools</span>
          </h1>
          <p className="text-xl text-gray-600">Start building today with our developer-friendly platform</p>
        </div>
